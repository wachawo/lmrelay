...
Si vous travaillez avec **Ollama**, vous rencontrez ceci : par défaut, elle n'est accessible que
depuis localhost et n'embarque aucune authentification. Se connecter à Ollama depuis une autre
machine impose en général de modifier sa configuration systemd, ou de placer un reverse proxy
devant. **lmrelay** répond à cela. Il s'installe avec `pip` et tourne en démon à côté d'Ollama :
il écoute sur un port qui lui est propre et, si vous le souhaitez, exige des identifiants.
...
Ou la branche `main` actuelle, qui peut devancer la version publiée :
...
### Limiter ce qu'un appelant peut demander
...
appelant ne peut pas le voir. `per_token` à côté empêche un client à cinquante fils de tout
accaparer.

Un appelant refusé reçoit un 429 qui nomme la portée, et un `Retry-After` quand le relais
...
compris, puis signale un relais en fonctionnement.
...
`export` accepte `--no-secrets`, lui et `import` acceptent `--force` pour écrire par-dessus
ce qui est déjà là, et sans aucun chemin le paquet sort sur stdout et est lu depuis stdin, si
bien que `lmrelay export | ssh other-host lmrelay import` déménage un relais en une ligne.
...
Deux étapes chacun : mettre la clé dans une variable, puis passer la variable à
`provider add`. La commande que vous copiez ne porte alors aucun secret, et la clé est
collée une fois au lieu d'une fois par fournisseur. Un `VAR=value` tapé à l'invite reste
dans l'historique du shell, donc `read -rs VAR` est la forme à utiliser quand cela compte.

Un nom qu'un preset connaît n'a pas besoin d'URL. Tout le reste a besoin de `--base-url`,
et le relais le dit au lieu de deviner.

**Ollama** n'a besoin d'aucune commande. La `lmrelay.toml` livrée porte déjà
`[upstream.ollama]` sur `http://127.0.0.1:11434` et en fait le `default_upstream`. Ne le
nommez que pour le déplacer sur une autre machine :

```bash
lmrelay provider add ollama "" --base-url http://gpu102:11434
```

Le token vide est voulu : le preset Ollama ne porte aucun en-tête, car un Ollama local ne
demande pas d'identifiant.

**OpenRouter** n'a pas de preset, il reçoit donc son URL. `openai` est le dialecte par
défaut et celui que parle OpenRouter, `--dialect` est donc inutile :
...
**OpenAI** est un preset : l'URL, le dialecte et la forme des en-têtes viennent avec le
nom :
...
**Gemini** parle le dialecte OpenAI, mais sur un chemin qui lui est propre et non sous
`/v1`. L'URL de base est donc l'hôte nu, et `/v1beta/openai` est porté par le client :
...
**Claude** est le preset `anthropic`, et c'est tout l'intérêt du preset : il envoie
`x-api-key` et `anthropic-version`, qui est ce avec quoi cette API authentifie. Un nom à
vous recevrait à la place le `Authorization: Bearer` par défaut, et les valeurs de
`--header` sont prises littéralement : un `{token}` écrit là part comme ces sept
caractères et non comme la clé :
...
Relisez ensuite ce que le relais utilisera réellement, clés masquées sauf demande
contraire :
...
`python3 main.py run` lance le relais directement depuis le checkout, sans l'installer ; pour cela `requirements.txt` seul suffit.
...
[limites](https://github.com/wachawo/lmrelay/blob/main/docs/CONFIGURATION.md#limits) sur trois portées : par identifiant, par adresse et pour le
relais entier ; la première est rattachée au jeton de l'appelant, ce que nginx ne sait pas
faire sans détenir lui-même les jetons. Elles sont comptées dans ce seul processus. Les deux se
composent au lieu de se concurrencer. Mettez nginx devant pour TLS, et laissez ici les jetons,
les fournisseurs et les limites.
### Licence
