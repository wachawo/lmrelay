...
- Windows выполняет только `run`. `serve` сообщает, что на этой платформе нет `os.fork`, а
  `enable` сообщает, что нет ни systemd, ни launchd, вместо того чтобы запуститься наполовину.
- Локальная Ollama на 11434 идёт апстримом по умолчанию, но она не обязательна. Релей, где
  настроены только облачные провайдеры, тоже допустима, если `default_upstream`
...
### Сколько может просить один вызывающий
...
не видит. `per_token` рядом не даёт одному клиенту с пятьюдесятью потоками занять всё.

Отклонённый вызывающий получает 429 с именем области и `Retry-After`, когда релей может
...
включительно, а затем сигналит работающему релею.
...
принимает `--no-secrets`, она и `import` принимают `--force`, чтобы записать поверх того,
что уже есть; а совсем без пути бандл уходит в stdout и читается из stdin, так что
`lmrelay export | ssh other-host lmrelay import` переносит релей одной строкой. `--config PATH`
...
По два шага на каждого: положить ключ в переменную, затем передать переменную в
`provider add`. Команда, которую вы копируете, не несёт секрета, а ключ вставляется один
раз, а не по разу на провайдера. Набранное в приглашении `VAR=value` всё равно попадает в
историю оболочки, поэтому там, где это важно, форма такая: `read -rs VAR`.

Имени, которое знает пресет, URL не нужен. Всему остальному нужен `--base-url`, и relay об
этом говорит, а не догадывается.

**Ollama** не требует команды вовсе. Штатный `lmrelay.toml` уже несёт
`[upstream.ollama]` на `http://127.0.0.1:11434` и делает его `default_upstream`.
Называйте его только чтобы перенести на другую машину:

```bash
lmrelay provider add ollama "" --base-url http://gpu102:11434
```

Пустой токен здесь намеренно: пресет Ollama не несёт заголовков, потому что локальная
Ollama не спрашивает учётных данных.

**OpenRouter** пресета не имеет, поэтому получает свой URL. `openai` - диалект по
умолчанию и как раз тот, на котором говорит OpenRouter, так что `--dialect` не нужен:
...
**OpenAI** - пресет, поэтому URL, диалект и форма заголовков приходят вместе с именем:
...
**Gemini** говорит на диалекте OpenAI, но по своему пути, а не под `/v1`. Поэтому базовый
URL - это голый хост, а `/v1beta/openai` несёт клиент:
...
**Claude** - это пресет `anthropic`, и пресет здесь и есть суть: он отправляет
`x-api-key` и `anthropic-version`, а именно этим то API и аутентифицирует. Своё имя
получило бы вместо них `Authorization: Bearer` по умолчанию, а значения `--header` берутся
буквально, так что написанный там `{token}` уйдёт этими семью символами, а не ключом:
...
Затем прочитайте обратно, что relay действительно будет использовать, ключи
замаскированы, если не попросить иначе:
...
`GET /metrics` отвечает на скрейп Prometheus агрегированными счётчиками и учётные данные как
раз требует, потому что говорит, как релей используют, а не только что он жив. Всё остальное
идёт через релей.
...
nginx и так умеет reverse proxy, поэтому демон обязан оправдать своё место. Коротко, по
пунктам:

- **Ключи провайдеров оказываются внутри `nginx.conf`.** По `location` и
  `proxy_set_header Authorization "Bearer sk-..."` на каждого, плюс
  `proxy_ssl_server_name on`, когда апстрим говорит по TLS. Здесь это одна команда, а ключ
  лежит в файле `0600`, а не в `0644`, принадлежащем root.
- **Проверка токена клиента в nginx кладёт туда же и сами токены.** `map` и `internal`
  `location` справляются без бэкенда, но каждый токен становится строкой открытым текстом в
  том же root-файле, а выдача или отзыв требуют правки и перезагрузки.
- **У `htpasswd` нет ни id, ни ротации.** У `lmrelay token gen --label laptop`, `token list`
  и `token delete 1` есть.
- **Значения nginx по умолчанию ломают стриминг.** `proxy_buffering` включён, а
  `proxy_read_timeout` равен 60s, тогда как большая локальная модель может думать дольше
  минуты до первого токена. Оба параметра приходится найти и выключить, обычно уже после
  того, как ответ оборвался на середине.
- **Путь чужого диалекта через nginx возвращает 404 самого провайдера.** Для форм, которые
  релей распознаёт, например пути Anthropic, отправленного в апстрим OpenAI, он отвечает 400
  своими словами, и ошибку не принимают за ответ провайдера.
- **nginx не поставляется ни с macOS, ни с Windows.** `pip install` работает одинаково на
  обеих.
- **SDK нельзя направить на `auth_basic` документированным способом.** Тот принимает `Basic`
  и отвергает всё остальное, тогда как каждый SDK кладёт свой ключ в
  `Authorization: Bearer`. Учётные данные в URL действительно проходят, но тогда `api_key`
  становится балластом: httpx записывает данные из URL в тот же заголовок, и bearer никуда не
  уходит. Каждый пример из документации провайдера приходится переписывать.
