...
В чём выигрывает nginx: TLS, настоящее ограничение частоты запросов и то, что он уже
установлен. У lmrelay нет ни одного из трёх, и не будет. Они дополняют друг друга, а не
соперничают. Поставьте nginx впереди ради TLS, а токены и провайдеров оставьте здесь.
### Лицензия
